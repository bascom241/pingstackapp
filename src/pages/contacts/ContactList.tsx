...
import { Users, Plus, Search, Filter, ArrowUpRight, Upload, Trash2, MoreVertical, FileText, CheckCircle2 } from "lucide-react";
...
export default function Contacts() {
  const [activeList, setActiveList] = useState<string>("l1");
  const [searchQuery, setSearchQuery] = useState("");
  const [isNewListModalOpen, setIsNewListModalOpen] = useState(false);
  const [newListName, setNewListName] = useState("");

  // Mock Segmented Contact Lists
  const [lists, setLists] = useState<ContactList[]>([
    { id: "l1", name: "Nigeria Production VIPs", count: 1420, status: "Active", updatedAt: "2 mins ago" },
    { id: "l2", name: "OTP Verification Fallbacks", count: 89, status: "Active", updatedAt: "1 hr ago" },
    { id: "l3", name: "Black Friday Promos 2026", count: 12450, status: "Draft", updatedAt: "2 days ago" },
    { id: "l4", name: "Stale / Inactive Carriers", count: 310, status: "Archived", updatedAt: "1 week ago" },
  ]);
  ...
  const currentContacts = contactsData[activeList] || [];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 items-start">
        ...
        <div className="lg:col-span-2 space-y-3">
          <span className="text-[11px] font-bold uppercase tracking-wider text-gray-400">
            Available Databases ({lists.length})
          </span>
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                      list.status === "Active" ? "bg-emerald-50 text-emerald-700 border border-emerald-100" :
        ...
        <div className="lg:col-span-3 bg-white border border-gray-100 rounded-2xl p-5 shadow-sm min-w-0 flex flex-col min-h-[480px]">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-gray-50 mb-4">
            ...
            <div className="flex items-center gap-2 justify-end">
              ...
              <button className="bg-gray-50 border border-gray-200 hover:border-gray-300 text-gray-700 font-bold text-xs px-3 py-2 rounded-xl flex items-center gap-1 cursor-pointer transition-colors">
              ...
              <table className="min-w-full divide-y divide-gray-100">
                ...
                <tbody className="divide-y divide-gray-50 text-xs text-gray-600 font-medium">
                  {currentContacts
                    ...
                    .map((contact) => (
                      <tr key={contact.id} className="hover:bg-gray-50/40 transition-colors group">
                        {/* Identity Pillar */}
                        <td className="py-3">
                        ...
                        <td className="py-3">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold ${
                            contact.status === "Subscribed" ? "bg-emerald-50 text-emerald-700 border border-emerald-100" :
