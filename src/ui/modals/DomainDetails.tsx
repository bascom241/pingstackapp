// src/components/email/DomainDetailsModal.tsx
...
import type { BrevoDomainDto, DnsRecordDetails } from "../../types/email/EmailConfigDto";
import {
  useGetSingleDomain,
...
export const DomainDetailsModal: React.FC<DomainDetailsModalProps> = ({
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape" && !isSettingPrimary && !isVerifying) onClose();
    };
    window.addEventListener("keydown", handleKeyDown);
    return () => window.removeEventListener("keydown", handleKeyDown);
  }, [onClose, isSettingPrimary, isVerifying]);
            ...
            <div className="flex items-center gap-2 mt-3 sm:mt-3">
              ...
              {!isPrimaryDomain && (
                ...
                >
                  ...
                  <span>{isSettingPrimary ? "Setting Primary..." : "Set Primary"}</span>
                </button>
              )}
            </div>
          )}
